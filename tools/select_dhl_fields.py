...
def select_rate_card_fields(file_path: str) -> str:
    ...
    try:
        ...
        # Do not show these mappings in frontend response (but still store them for backend usage if needed)
        EXCLUDE_FROM_FRONTEND = {"DAYS", "SERVICE_CODE","LANE"}

        for item in matched_cols:
            if "system_field_key" in item and "excel_column" in item:
                sys_key = item["system_field_key"]
                excel_col = item["excel_column"]
                actual_col = _actual_column(excel_col)

                # Always keep in stored mapping (no functionality change)
                column_mapping[sys_key] = actual_col

                # But do not expose these two in frontend response
                if sys_key in EXCLUDE_FROM_FRONTEND:
                    continue

...
# Single-batch mode: exactly one $batch request; allow up to 600 entries in that one request.
MAX_SINGLE_BATCH_ENTRIES = 600
...
def batch_upload_to_sap(
    ...
    try:
        ...
        # -------------------------------------------------------
        # BATCH SEND (SINGLE REQUEST) + SAVE FULL BATCH TO .TXT
        # -------------------------------------------------------
        batch_boundary = "batch_123453"
        changeset_boundary = "changeset_all"
        response = None
        ...
        # Force exactly one batch: if > 600 entries, keep first 600 (single batch only)
        if len(all_entries) > MAX_SINGLE_BATCH_ENTRIES:
            logger.warning(
                f"{SAP_UPLOAD_LOG_PREFIX} Prepared %s entries but single-batch mode allows only %s. "
                f"Uploading ONLY the first %s entries (one batch).",
                len(all_entries),
                MAX_SINGLE_BATCH_ENTRIES,
                MAX_SINGLE_BATCH_ENTRIES,
                extra={
                    "prepared_entries": len(all_entries),
                    "max_single_batch_entries": MAX_SINGLE_BATCH_ENTRIES,
                },
            )
            all_entries = all_entries[:MAX_SINGLE_BATCH_ENTRIES]

        logger.info(
            f"{SAP_UPLOAD_LOG_PREFIX} Beginning SAP batch upload over HTTP (single request).",
            extra={
                "total_entries": len(all_entries),
                "max_retries": BATCH_MAX_RETRIES,
                "validity_uuid": validity_uuid,
        ...
        # Build ONE batch body (LF to match bash script)
        crlf = "\n"
        chunk_parts = []
        ...
        # Save full batch request (headers + body) before sending (no effect on response message)
        try:
            out_dir = os.path.dirname(excel_file_path) or os.getcwd()
            ts = time.strftime("%Y%m%d_%H%M%S")
            safe_session = re.sub(r"[^A-Za-z0-9_-]+", "_", str(session_id or "nosession"))
            safe_validity = re.sub(r"[^A-Za-z0-9_-]+", "_", str(validity_uuid or "novalid"))
            out_name = f"sap_batch_{safe_validity}_{safe_session}_{ts}.txt"
            out_path = os.path.join(out_dir, out_name)

            dump = (
                f"POST {SAP_BATCH_ENDPOINT.rstrip('/')} HTTP/1.1{crlf}"
                f"Content-Type: multipart/mixed;boundary={batch_boundary}{crlf}"
            ...
            logger.info(
                f"{SAP_UPLOAD_LOG_PREFIX} Batch request written to file.",
                extra={"batch_dump_path": out_path, "entries_in_batch": len(all_entries)},
            )
        except Exception as dump_exc:
            logger.warning(
                f"{SAP_UPLOAD_LOG_PREFIX} Could not write batch dump file; proceeding to upload. Error=%s",
                dump_exc,
                exc_info=True,
            )

        # Send single batch with retries
        for attempt in range(1, BATCH_MAX_RETRIES + 1):
            try:
                logger.info(
                    "Uploading SINGLE SAP batch (attempt %s of %s).",
                    attempt,
                    BATCH_MAX_RETRIES,
                    extra={
                        "attempt": attempt,
                        "max_retries": BATCH_MAX_RETRIES,
                        "entries_in_batch": len(all_entries),
                        "validity_uuid": validity_uuid,
                    },
                )
                ...
                logger.info(
                    "Successfully uploaded SINGLE SAP batch (status=%s).",
                    response.status_code,
                    extra={
                        "status_code": response.status_code,
                        "entries_in_batch": len(all_entries),
                ...
                resp_text = _safe_response_text(resp) if resp else str(http_exc)
                logger.warning(
                    f"{SAP_UPLOAD_LOG_PREFIX} Error uploading SINGLE SAP batch. Will retry if attempts remain. | status=%s | response_body=%s",
                    getattr(resp, "status_code", None) if resp else "N/A",
                    resp_text,
                    extra={
                        "attempt": attempt,
                ...
                if attempt == BATCH_MAX_RETRIES:
                    logger.error(
                        f"{SAP_UPLOAD_LOG_PREFIX} Exhausted all retry attempts for SINGLE SAP batch. | last_status=%s | response_body=%s",
                        getattr(resp, "status_code", None) if resp else "N/A",
                        resp_text,
                        extra={
                            "entries_in_batch": len(all_entries),
        ...
        if clear_status == "cleared":
            ...
            clear_note = (
                f" Clear before upload was not run ({clear_detail}). "
                "The agreement and rates have been successfully created. Please review and advise if these may be released for business use."
            )
        ...
        # IMPORTANT: do NOT mention saving batch in the returned response (per requirement)
        return (
            f"SUCCESS: Uploaded {len(all_entries)} rate entries to SAP. Summary: {summary_line}. "
            f"Weight breaks: {weights_list}."
            f"{clear_note}{view_note}"
        )

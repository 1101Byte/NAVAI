...
async def chat_endpoint(
    ...
    """Frontend-friendly alias for /invoke. Reuses the same logic to avoid duplication.

    This simply forwards the request to the existing `invoke` handler so behavior
    remains identical and existing functionality is unaffected.
    """
...
async def invoke(
    ...
    if file:
        try:
            ...
            file_path = os.path.abspath(
                os.path.join(TEMP_UPLOADS_DIR, f"{session_id}_{sanitized_name}")
            )
            ...
            logger.error(f"Error saving uploaded file: {e}", exc_info=True)
            actual_message += f" [FILE_UPLOAD_ERROR: {str(e)}]"
    ...
    try:
        ...
        user_message = types.Content(
            role="user",
            parts=[types.Part.from_text(text=actual_message)],
        )
...
async def chat_endpoint(
    ...
    """
    Frontend-friendly alias for /invoke.
    Reuses same logic.
    """
    ...
    # Call the existing invoke function and return its result
    return await invoke(session_id=session_id, message=message, file=file)
